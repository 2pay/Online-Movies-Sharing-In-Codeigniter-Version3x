	...
	<form name="register_form" action="<?php echo base_url().'register/yes'; ?>" method="post">
		<table width="90%" border="0" cellspacing="3" cellpadding="3" align="center">
			...
			<tr>
				...
				<td>
					<select name="birth_day_sel"><option value="">Day</option><?php for($i=1; $i<=31; $i++){ $k=(strlen($i)>1)?"":"0"; ?>
					...
					<select name="birth_year_sel"><option value="">Year</option><?php for($j=1900; $j<=2012; $j++){ ?><option value="<?php echo $j; ?>" <?php if($j==$this->input->post('birth_year_sel')){ ?>selected="selected"<?php } ?>><?php echo $j; ?></option><?php } ?>
					</select>
				</td>
			</tr>
			<tr>
